...
export async function runLlmRca(req: RcaLlmRequest): Promise<RcaLlmResult> {
  ...
  const maxInputTokens = req.maxInputTokens ?? 10_000;
  const maxOutputTokens = req.maxOutputTokens ?? 1_500;
  const topK = req.topK ?? 10;
  const maxBodyLines = req.maxBodyLines ?? 30;

  const subset = req.candidates.slice(0, topK);
  const bundles: CandidateBundle[] = subset.map((c, i) => {
    ...
    if (c.file && c.line != null) {
      ...
      const defs = definitionOf(req.db, c.name);
      ...
      body = fetchBody(req.repoRoot, c.file, startLine, endLine, maxBodyLines);
    }
    const callerTree = callersOf(req.db, c.name, { depth: 1, minConfidence: 0.5 });
    const calleeTree = calleesOf(req.db, c.name, { depth: 1 });
    return {
      rank: i + 1,
      candidate: c,
      body,
  ...
  // "below the cap."
  let trimmed = false;
  let user = renderUserPrompt(req.failureDescription, bundles);
  while (
    approxTokens(SYSTEM_PROMPT) + approxTokens(user) > maxInputTokens &&
    bundles.length > 0
  ) {
    trimmed = true;
    ...
    user = renderUserPrompt(req.failureDescription, bundles);
  }
